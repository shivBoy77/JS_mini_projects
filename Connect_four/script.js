...
function checkBottom(colIndex) {
  ...
  for (var row = 5; row > -1; row--) {
    ...
    if (colorReport === 'rgb(245, 245, 220)') {
      return row
    }
  }
}
...
function colorMatchCheck(one,two,three,four){
  return (one===two && one===three && one===four && one !== 'rgb(245, 245, 220)' && one !== undefined);
}
...
function gameEnd(winningPlayer) {
  for (var col = 0; col < 7; col++) {
    for (var row = 0; row < 7; row++) {
      $('h3').fadeOut('fast');
      $('h2').fadeOut('fast');
      $('h1').text(winningPlayer+" has won! Refresh your browser to play again!").css("fontSize", "45px")
